...
from ._common import _IGNORE, _rel, _truncate, _vendor_ignore_regex
...
def check_imports(state: Annotated[ReviewState, InjectedState]) -> str:
    ...
    that import each other). Does NOT execute the project's code — the analysis
    is purely static (AST + import-spec lookup)."""
...
# ── import health check: broken / unused / circular imports ──────────────
# All static (ast + import-spec lookup) — no project code is executed. The
# checks are deliberately conservative: when in doubt we DON'T flag, because a
# false "broken import" is more harmful to a review than a missed one.
...
def _check_imports(path: str, language: str) -> str:
    """Route the import check to the right per-language implementation."""
    if language == "python":
        return _check_imports_python(path)
    # ── extension point: add JS/TS/Go/… implementations here ──────────────
    return (f"Import checking is not yet implemented for {language}. "
